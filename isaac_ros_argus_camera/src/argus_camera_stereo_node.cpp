...
constexpr char OUTPUT_COMPONENT_KEY_LEFT_CAMERAINFO[] =
    "sink_left_camera_info/sink";
constexpr char OUTPUT_TOPIC_NAME_LEFT_CAMERAINFO[] = "left/camera_info";

constexpr char OUTPUT_COMPONENT_KEY_RIGHT_CAMERAINFO[] =
    "sink_right_camera_info/sink";
...
const std::vector<std::pair<std::string, std::string>> EXTENSIONS = {
    ...
    {"gxf_isaac_timestamp_correlator",
     "gxf/lib/libgxf_isaac_timestamp_correlator.so"},
    {"gxf_isaac_argus", "gxf/lib/libgxf_isaac_argus.so"},
    {"gxf_isaac_message_compositor",
     "gxf/lib/libgxf_isaac_message_compositor.so"},
...
ArgusStereoNode::ArgusStereoNode(const rclcpp::NodeOptions& options)
    : ArgusCameraNode(options, APP_YAML_FILENAME, CONFIG_MAP,
  ...
  left_optical_frame_name_ =
      declare_parameter<std::string>("left_optical_frame_name", "left_cam");
  ...
  if (!left_camera_info_url_.empty()) {
    ...
    RCLCPP_INFO(get_logger(),
                "[ArgusStereoNode] Loaded left camera info from \"%s\"",
  ...
  if (!right_camera_info_url_.empty()) {
    ...
    RCLCPP_INFO(get_logger(),
                "[ArgusStereoNode] Loaded right camera info from \"%s\"",
                right_camera_info_url_.c_str());
  }

  // Adding callback for left image
  config_map_[OUTPUT_COMPONENT_KEY_LEFT_IMAGE].callback = std::bind(
      &ArgusCameraNode::ArgusImageCallback, this, std::placeholders::_1,
      std::placeholders::_2, left_optical_frame_name_);

  // Adding callback for right image
  config_map_[OUTPUT_COMPONENT_KEY_RIGHT_IMAGE].callback = std::bind(
      &ArgusCameraNode::ArgusImageCallback, this, std::placeholders::_1,
      std::placeholders::_2, right_optical_frame_name_);

  // Adding callback for left camera_info
  config_map_[OUTPUT_COMPONENT_KEY_LEFT_CAMERAINFO].callback = std::bind(
      &ArgusCameraNode::ArgusCameraInfoCallback, this, std::placeholders::_1,
      std::placeholders::_2, camera_link_frame_name_, left_optical_frame_name_,
      left_camera_info_);

  // Adding callback for right camera_info
  config_map_[OUTPUT_COMPONENT_KEY_RIGHT_CAMERAINFO].callback = std::bind(
      &ArgusCameraNode::ArgusCameraInfoCallback, this, std::placeholders::_1,
      std::placeholders::_2, camera_link_frame_name_, right_optical_frame_name_,
      right_camera_info_);
...
void ArgusStereoNode::postLoadGraphCallback() {
  ...
  if (wide_fov_) {
    getNitrosContext().setParameterFloat64(
        "rectify_parameters", "nvidia::isaac::RectifyParamsGenerator", "alpha",
        0.7);
    RCLCPP_INFO(
        get_logger(),
        "[ArgusStereoNode] set alpha in rectify parameter generator to  \"%f\"",
        0.7);
  }
}
