...
ArgusMonoNode::ArgusMonoNode(const rclcpp::NodeOptions& options)
  ...
  optical_frame_name_ =
      declare_parameter<std::string>("optical_frame_name", "left_cam");
  ...
  if (!camera_info_url_.empty()) {
    ...
    RCLCPP_INFO(get_logger(), "[ArgusMonoNode] Loaded camera info from \"%s\"",
  ...
  config_map_[OUTPUT_COMPONENT_KEY_LEFT_IMAGE].callback = std::bind(
      ...
      std::placeholders::_2, optical_frame_name_);

  // Adding callback for left camera_info
  config_map_[OUTPUT_COMPONENT_KEY_LEFT_CAMERAINFO].callback =
      std::bind(&ArgusCameraNode::ArgusCameraInfoCallback, this,
                std::placeholders::_1, std::placeholders::_2,
                camera_link_frame_name_, optical_frame_name_, camera_info_);
